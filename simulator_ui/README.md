...
- **Inter-Grid Transfers**: Bins can be moved between grids via conveyors. Each grid is connected to other grids via a conveyor belt.
...
- **Queue Rollback**: When a port closes, any shipments still in its local queue are returned to the general Grid Queue.
- **Router Rollback**: Shipments that are `Routed`,`Ready` or `Consolidation` but not yet started picking or have active transfers are rolled back to `Received`, their bin reservations cleared before each router run.
...
shipment idedi i porta
shipment tampa consolidated
